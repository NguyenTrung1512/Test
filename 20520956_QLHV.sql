...
-- Câu 1 -- 
select GIAOVIEN.HOTEN, GIAOVIEN.MAGV, GIAOVIEN.HESO, KHOA.TRGKHOA from GIAOVIEN join KHOA on GIAOVIEN.MAGV = KHOA.TRGKHOA
update GIAOVIEN set HESO = HESO + 0.2 FROM GIAOVIEN join KHOA on GIAOVIEN.MAGV = KHOA.TRGKHOA


-- Câu 3 --
SELECT * from HOCVIEN, KETQUATHI where (KETQUATHI.LANTHI = 3 and KETQUATHI.DIEM < 5 AND KETQUATHI.MAHV = HOCVIEN.MAHV)
update HOCVIEN set GHICHU = 'Cam Thi' FROM HOCVIEN, KETQUATHI WHERE (KETQUATHI.LANTHI = 3 and KETQUATHI.DIEM < 5 and KETQUATHI.MAHV = HOCVIEN.MAHV)
...
-- cách 2 --
SELECT HV.MAHV, HO + ' ' + TEN AS HOTEN, MH.TENMH
...
WHERE HV.MALOP LIKE 'K%' 
	  ...
	  AND NOT EXISTS (
					 	SELECT *
						FROM KETQUATHI 
						WHERE KQUA = 'Dat' AND HV.MAHV = KETQUATHI.MAMH AND MAMH = 'CTRR'
					 )		 
...
WHERE GD.HOCKY = 1 
	  AND GD.NAM = 2006
	  AND GD.MAGV in (SELECT MAGV FROM GIAOVIEN WHERE GIAOVIEN.HOTEN = 'Tran Tam Thanh')
...
WHERE GD.HOCKY = 1
	  AND GD.NAM = 2006
	  AND GD.MAGV = (SELECT MAGV FROM LOP WHERE MALOP = 'K11')
...
WHERE HV.MALOP = ( SELECT GD.MALOP 
				   FROM GIANGDAY GD JOIN MONHOC MH
				   ON GD.MAMH = MH.MAMH 
				   WHERE MH.TENMH = 'Co So Du Lieu' AND GD.MAGV = ( 
																	SELECT MAGV
																	FROM GIAOVIEN GV
...
WHERE (
	   KQ.MAMH = 'CSDL' 
	   AND KQ.LANTHI = 1 
	   AND KQ.KQUA = 'Khong Dat') AND NOT EXISTS (
											       SELECT *
												   FROM KETQUATHI KQ
												   WHERE LANTHI > 1 AND KQ.MAHV = HV.MAHV
												 )
...
WHERE NOT EXISTS (
				   SELECT * 
				   FROM GIANGDAY GD JOIN MONHOC MH
				   ON GD.MAMH = MH.MAMH AND GV.MAGV = GD.MAGV
...
WHERE MALOP = 'K11' AND 
					(
					  EXISTS ( SELECT * FROM KETQUATHI KQ WHERE LANTHI > 3 AND KQUA = 'Khong Dat' AND HOCVIEN.MAHV = KQ.MAHV)
					  OR 
					  EXISTS ( SELECT * FROM KETQUATHI KQ WHERE KQ.MAMH = 'CTRR' AND KQ.LANTHI = 2 AND KQ.DIEM = 5 AND HOCVIEN.MAHV = KQ.MAHV)
					)
...
WHERE KQ.MAMH = 'CSDL' AND LANTHI = ( 
										SELECT MAX(LANTHI)
										FROM KETQUATHI 
										WHERE HV.MAHV = KETQUATHI.MAHV AND KETQUATHI.MAMH = 'CSDL'
										GROUP BY MAHV 
									)
